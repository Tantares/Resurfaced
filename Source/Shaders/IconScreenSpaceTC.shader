Shader "Resurfaced/ScreenSpaceIcon (TC)"
{
  Properties
  {
      ...
      _MinX("Clip Min X", Range(0,1)) = 0.0
      _MaxX("Clip Max X", Range(0,1)) = 1.0
      _MinY("Clip Min Y", Range(0,1)) = 0.0
      _MaxY("Clip Max Y", Range(0,1)) = 1.0
    ...
    SubShader
      {
          ...
          void surf(Input IN, inout SurfaceOutputStandard o)
          {
            ...
            float2 coords = IN.screenPos.xy / IN.screenPos.w;
              clip(coords.x < _MinX | coords.x >_MaxX ? -1 : 1);
              clip(coords.y < _MinY | coords.y >_MaxY ? -1 : 1);
